...
exports.createMomoPayment = async (req, res) => {
  ...
  const redirectUrl = ""; // Không dùng nhưng bắt buộc có
  const ipnUrl = `${process.env.BASE_URL}/api/momo/ipn`;
  ...
  const requestBody = JSON.stringify({
    ...
    orderId,
    orderInfo,
    redirectUrl, // cần có dù không dùng
    ipnUrl,
    extraData,
    requestType,
    signature,
  ...
  const momoReq = https.request(options, (momoRes) => {
    ...
    momoRes.on("end", () => {
      ...
      res.json(result); // Trả về deeplink hoặc payUrl
    });
  });
...
exports.handleMomoIPN = async (req, res) => {
  ...
  if (resultCode == 0) {
    ...
    try {
      ...
      await db.query(
        `UPDATE payment SET Payment_date = NOW(), Transaction_id = ?, Updated_at = NOW()
...
// API hủy đơn chưa thanh toán sau 10 giây (test)
exports.autoCancelUnpaidOrders = async (req, res) => {
  try {
    const [rows] = await db.query(
      `UPDATE \`order\`
       SET Order_status = 'Đã hủy', Payment_status = 'Thất bại', Updated_at = NOW()
